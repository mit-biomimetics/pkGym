...
class MIT_humanoid(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # * ref trajectory
        # Grabs ref traj data from CSV - assumes it's in "./trajectories/humanoid3d_walk.csv"
        reference_traj = pd.read_csv("./trajectories/humanoid3d_run.csv")
        totalTime = reference_traj['t'].iloc[-1]
        # Scale times [sec] to standard phase 0->1
        reference_traj['t'] /= totalTime
        reference_traj = np.ascontiguousarray(reference_traj.to_numpy(dtype='float32'))
        # env.setReferenceTrajectory(reference_traj, totalTime.item())
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        asset_root = "urdf"
        asset_file = "mit_humanoid/humanoid_R_sf.urdf"
...
def compute_humanoid_reward(
    ...
    # smoothness
    rew_smooth = torch.sum(torch.exp(-0.25*torch.square(actions
                                                   -2*actions_k1
                                                   +actions_k2)), \
                                                   dim=1)*rew_scales["smooth"]
    ...
    # symmetry
    # pitch left: [2, 3, 4, 10, 13]
    # pitch right: [7, 8, 9, 14, 17]
    sym_pos = torch.square(dof_pos[:, [2, 10, 13]] +
                                     dof_pos[:, [7, 14, 17]])
    sym_vel = torch.square(dof_vel[:, [2, 10, 13]] +
                                     dof_vel[:, [7, 14, 17]])

    sym_pos = torch.cat((sym_pos, torch.square(dof_pos[:, [0, 1]] -
                                     dof_pos[:, [5, 6]])), dim=1)

    rew_sym = torch.sum(torch.exp(-sym_pos), dim=1) + \
        torch.sum(torch.exp(-sym_vel), dim=1) * \
        rew_scales["symmetry"]

    total_reward = rew_lin_vel_xy + rew_ang_vel_z + rew_torque + \
        rew_smooth + rew_height + rew_sym
