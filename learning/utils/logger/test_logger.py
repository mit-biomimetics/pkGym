...
def only_rewards_registered(logger, reward_names):
    for key in logger.reward_logs.log_items.keys():
        assert key in reward_names, \
            "erroneous key in logger."
...
def test_logger_setup():
    ...
    logger.register_category(category='alg_performance',
                             target=mocky,
                             attribute_list=['a', 'b'])
...
def test_logging_rewards():
    ...
    check_average_time(logger, expected_time=1.)

    check_average_reward(logger, 'first', expected_average=5.)
    check_average_reward(logger, 'second', expected_average=1.)
    check_average_reward(logger, 'third', expected_average=0.)


def test_logging_iteration():
    mocky = MockObject()
    logger = Logger()
    logger.initialize(num_envs=2,
                      episode_dt=0.1,
                      total_iterations=1000,
                      device='cpu')
...
def test_timer():
    ...
    logger = Logger()
    logger.initialize(total_iterations=1000,
                      device='cpu')
    logger.tic('first_step')
    assert logger.get_time('first_step') == -1, \
