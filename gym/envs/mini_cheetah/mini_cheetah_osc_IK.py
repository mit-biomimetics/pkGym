...
class MiniCheetahOscIK(MiniCheetahOsc):
    def _init_buffers(self):
        super()._init_buffers()
        self.ik_pos_target = torch.zeros(
    ...
    def _pre_torque_step(self):
        ...
        joints_ik_fr = self.IK_leg_3DOF(
            p_hr2ft_hr=self.ik_pos_target[:, 0:3] + ik_defaults_R
        )
        joints_ik_fl = self.IK_leg_3DOF(
            p_hr2ft_hr=self.ik_pos_target[:, 3:6] + ik_defaults_L
        )
        joints_ik_br = self.IK_leg_3DOF(
            p_hr2ft_hr=self.ik_pos_target[:, 6:9] + ik_defaults_R
        )
        joints_ik_bl = self.IK_leg_3DOF(
            p_hr2ft_hr=self.ik_pos_target[:, 9:12] + ik_defaults_L
        )
    ...
    def IK_leg_3DOF(self, p_hr2ft_hr):
        ...
        r_hr2hp = (
            torch.tensor([0.0, -0.019, 0.0], device=self.device)
            .unsqueeze(0)
            .repeat(self.num_envs, 1)
        )
        r_hp2kp = (
            torch.tensor([0.0, 0.0, -0.2085], device=self.device)
            .unsqueeze(0)
            .repeat(self.num_envs, 1)
        )
        r_kp2ft = (
            torch.tensor([0.0, 0.0, -0.22], device=self.device)
            .unsqueeze(0)
            .repeat(self.num_envs, 1)
        )
        ...
        L_hp2ft_hr = torch.clamp(torch.norm(p_hr2ft_hr - p_hr2hp_hr, dim=1), min=1e-6)
        vec = p_hr2ft_hr - p_hr2hp_hr
        p_hp2ft_hp = torch.stack(
            (
                vec[:, 0],
                vec[:, 1] * torch.cos(q1) + vec[:, 2] * torch.sin(q1),
                vec[:, 2] * torch.cos(q1) - vec[:, 1] * torch.sin(q1),
            ),
            dim=1,
        )
